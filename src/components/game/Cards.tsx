...
export default function Card({ card, onClick, className = "w-28" }: CardProps) {
    return(
        <motion.button whileHover={{scale:card.isFlipped ? 1 : 1.04, y:card.isFlipped ? 0 :-4,}} whileTap={{scale:0.96,}}transition={{type:"spring",stiffness:400,damping:20,}}onClick={()=>onClick(card.id)} className={`relative aspect-square [perspective:1000px] ${className}`}>
